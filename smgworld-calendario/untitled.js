...
jQuery(function() {
var $ = jQuery, cal, fmtData, hscEl, modalShown = false;
...
function convertDate(date) {
	var date = new Date(date).toISOString().split("T")[0].split("-").reverse();
...
function convertData(data) {
	...
	fmtData.forEach(function(evt,i) {
		...
		var text = (time ?  "<time>" + hsc(time) + "</time> " : time) + hsc(evt.title);
		text = '<a href="#" class="modalmaker" data-index="' + i + '">' + text + "</a>";
...
function updateMonthYear() {
	$.getJSON("http://smgapps.bu.edu/smgnet/smgcalendarU/json.cfm?month=" + cal.getMonth() + "&year=" + cal.getYear(),null,function(data) {
...
$("#calendar").on("click",".fc-calendar .fc-row > div > div a",function() {
	...
	if(data.weblink != "http://" && data.weblink) {
		$("#modal h1").html("<a></a>");
		$("#modal h1 a").text(data.title);
		$("#modal h1 a, #modal .link").attr("href",data.weblink);
		$("#modal h1 a, #modal .link").attr("target","_blank");
		$("#modal .link").show();
	} else {
		$("#modal h1").text(data.title);
		$("#modal .link").hide();
	}
	var time = convertTime(data.eventdate), date = data.eventdate.split(" ").slice(0,-1).join(" ").replace(",","");
	$("#modal .time").text((time ? time + " on " : time) + date);
...
$("body").click(function(e) {
	if($(e.target).parent("#modal").size() === 0 && !$(e.target).is(".fc-calendar .fc-row > div > div a, #modal") && modalShown) {
		$("#modal").hide();
		modalShown = false;
	}
});
...
$('#custom-current').click(function () { cal.gotoNow(updateMonthYear); });
